...
function classifyExtra(k: string, v: unknown): CustomField | undefined {
  if (typeof v === 'boolean') {
    return { key: k, value: String(v), type: 'boolean' };
  }
  if (typeof v === 'number') {
    return { key: k, value: String(v), type: 'number' };
  }
  if (typeof v === 'string') {
    return { key: k, value: v, type: 'string' };
  }
  return undefined;
}
...
export function hydrate(
  ...
  for (const [k, v] of Object.entries(seeded)) {
    ...
    if (extra) {
      ...
      passthrough[k] = v;
    } // list/table — preserved, hand-edited elsewhere
  }
...
export async function loadToml(
  ...
  if (acquisitionId) {
    params.set('acquisition_id', acquisitionId);
  }
...
export function errorsByPath(errors: TomlFieldError[]): Record<string, string> {
  const out: Record<string, string> = {};
  for (const e of errors) {
    const key = e.loc.join('.');
    if (!(key in out)) {
...
export function buildSectionedPayload(
  ...
  for (const s of sections) {
    ...
    if (s.repeatable) {
      ...
      const st =
        (state[s.section] as SectionState | undefined) ?? emptySection();
      ...
      else if (s.section === form || Object.keys(obj).length) {
        out[s.section] = obj;
      }
    }
  }
...
export async function fetchMdRunIds(sampleId: string): Promise<string[]> {
  const res = await fetch(
    `/api/toml/md-run-ids/${encodeURIComponent(sampleId)}`
  );
  if (!res.ok) {
    return [];
  }
  ...
  for (const s of sections) {
    ...
    if (s.repeatable) {
      const entries = (Array.isArray(st) ? st : [])
        ...
        .filter(o => Object.keys(o).length > 0);
      if (entries.length) {
        out[s.section] = entries;
      }
    } else {
      const obj = buildSectionObject(
        fields,
        (st as SectionEntry) ?? emptyEntry()
      );
      if (Object.keys(obj).length > 0 || s.section === primary) {
        out[s.section] = obj;
      }
    }
  }
...
function splitEntry(
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (f) {
      if (f.derived) {
        continue;
      } // derived (e.g. data_source) is not an input
...
export function hydrateComposite(
  ...
  for (const s of sections) {
    state[s.section] = s.repeatable ? [] : emptyEntry();
  }
  const passthrough: Record<string, unknown> = {};
  ...
  for (const [name, raw] of Object.entries(seeded)) {
    ...
    if (s.repeatable) {
      ...
      if (ds === 'experimental' || ds === 'simulation') {
        dataSource = ds;
      }
    }
  }
...
export function inferDataSource(parsed: Record<string, unknown>): InferResult {
  ...
  const sim =
    SIM_BLOCKS.some(b => blockPresent(parsed, b)) || ds === 'simulation';
  const exp =
    EXP_BLOCKS.some(b => blockPresent(parsed, b)) ||
    sampleField(parsed, 'project') === 'synapse' ||
    ds === 'experimental';
  if (sim && exp) {
    return { kind: 'ambiguous', reason: 'conflict' };
  }
  if (sim) {
