...
type Props = {
  ...
  readonly initialSampleId?: string;
...
function GroupedAuthoringForm({
  ...
  React.useEffect(() => {
    if (!initialSampleId || !initialAcquisitionId) {
      return;
    }
...
function pathFor(section: FormSection, field: string, index?: number): string {
  ...
  if (index !== undefined) {
    return `${section.section}.${index}.${field}`;
  }
...
function SectionedAuthoringForm({
  ...
  const [dataSource, setDataSource] =
    React.useState<DataSource>('experimental');
  ...
  React.useEffect(() => {
    if (!wantsMdRunIds || !sampleId.trim()) {
      return;
    }
  ...
  React.useEffect(() => {
    if (!wantsTiltSeriesIds || !sampleId.trim() || !acquisitionId.trim()) {
      return;
    }
  ...
  return (
    <Box component="form" noValidate onSubmit={handleSubmit}>
      <Stack spacing={2}>
        ...
        {gated ? (
          <DataSourceRadio onChange={setDataSource} value={dataSource} />
        ) : null}

        {seedError ? <Alert severity="error">{seedError}</Alert> : null}
        {catalogFallback ? <StaleValuesWarning /> : null}
...
function ScalarSection({
  ...
  readonly onChange: (field: string, v: string) => void;
  readonly onCustomChange: (next: CustomField[]) => void;
}) {
...
function useConfirmDelete() {
  ...
  const dialog = (
    <Dialog onClose={() => setPending(null)} open={pending !== null}>
...
function RepeatableSection({
  ...
  readonly section: FormSection;
  readonly fields: FormField[];
  readonly entries: SectionState[];
  readonly errors: Record<string, string>;
  readonly suggestions: Record<string, string[]>;
  readonly namespaces: Record<string, string[]>;
  readonly onAdd: () => void;
  readonly onRemove: (index: number) => void;
  readonly onChange: (index: number, field: string, v: string) => void;
}) {
  const idFieldName = fields.find(f => f.required)?.field;
    ...
    <Box>
      ...
      <Stack spacing={2}>
        {entries.map((entry, i) => {
          ...
          const ownId = idFieldName
            ? (entry.values[idFieldName] ?? '').trim()
            : '';
          return (
            <Box
              key={i}
              ...
              >
                ...
                {!locked ? (
                  <IconButton
                    ...
                    onClick={() =>
                      confirm(`Delete this ${section.title} entry?`, () =>
                        onRemove(i)
                      )
                    }
                    size="small"
...
function Field({
  ...
  readonly suggestions?: Record<string, string[]>;
  readonly crossRefOptions?: string[];
...
function CustomFields({
  ...
  readonly onChange: (next: CustomField[]) => void;
  ...
  const removeField = (i: number) => {
    const c = fields[i];
    if (c.key || c.value) {
      confirm('Delete this custom field?', () => remove(i));
    } else {
      remove(i);
    }
  };
  ...
  return (
    <Box sx={{ borderTop: 1, borderColor: 'grey.400', pt: 2 }}>
      ...
      <Stack spacing={1}>
        {fields.map((c, i) => (
          <Stack alignItems="center" direction="row" key={i} spacing={1}>
            <TextField
              label="Key"
...
function PlacementHint({
  ...
  return (
    <Typography sx={{ color: 'primary.main', fontWeight: 500 }} variant="body2">
      Save the downloaded file inside a {noun} folder named with the {noun} id.
      For example: <code>{placement}</code>.
    </Typography>
  );
}
...
function DataSourceRadio({
  ...
  readonly onChange: (v: DataSource) => void;
  readonly disabled?: boolean;
}) {
  return (
...
function UploadLoadToolbar({
  ...
  readonly onLoadIdChange: (v: string) => void;
      ...
      {showLoadById ? (
        <>
          <TextField
            ...
            value={loadId}
          />
          <Button onClick={onLoad} size="small" variant="outlined">
            Load
          </Button>
        </>
      ) : null}
...
function initState(form: FormKind): Record<string, CompositeSection> {
  ...
  for (const s of sectionsFor(form)) {
    out[s.section] = s.repeatable ? [] : emptyEntry();
  }
  return out;
}
...
function CompositeAuthoringForm({
  ...
  const requiredArm = PROJECT_REQUIRES_DATA_SOURCE[project] as
    | DataSource
    | undefined;
  ...
  const updateEntry = (
    ...
    setState(prev => {
      ...
      if (Array.isArray(cur)) {
        return {
          ...prev,
          [section]: cur.map((e, i) => (i === idx ? patch(e) : e))
        };
      }
  ...
  const fieldKey = (section: string, fieldName: string, idx?: number) =>
    idx === undefined
      ? `${section}.${fieldName}`
      : `${section}.${idx}.${fieldName}`;

  const visible = (s: FormSection) => {
    if (s.requiresProject === 'chromatin' && project === 'synapse') {
      return false;
    }
    if (s.requiresDataSource && s.requiresDataSource !== effectiveArm) {
      return false;
    }
    return true;
  };
