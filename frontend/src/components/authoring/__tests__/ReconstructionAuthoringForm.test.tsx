...
describe('AuthoringForm (reconstruction)', () => {
  ...
  it('follows a load-by-id in the selector, so switching back still works', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText(/Alignment software/)).toHaveValue('IMOD 4.12'),
    )
    expect(
      screen.getByText(
        'samp1/Position_86/Reconstructions/grp_a/reconstruction.toml',
  ...
  it('keeps hand-typed context when loading by id with no route context', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/Acquisition id/), 'Position_86')
    await userEvent.type(screen.getByLabelText(/Load from portal by id/), 'grp_a')
    await userEvent.click(screen.getByRole('button', { name: 'Load' }))
  ...
  it('loads the edited acquisition context, not the route acquisition', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/Acquisition id/), 'Position_99')
    await userEvent.type(screen.getByLabelText(/Load from portal by id/), 'grp_b')
    await userEvent.click(screen.getByRole('button', { name: 'Load' }))
  ...
  it('starts an empty file for a new group, showing the folder-name hint', async () => {
    ...
    await userEvent.click(screen.getByRole('combobox', { name: /Group/ }))
    await userEvent.click(await screen.findByRole('option', { name: /New group/ }))
