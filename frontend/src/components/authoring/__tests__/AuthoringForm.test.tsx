  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation(((
    ...
    if (url.includes('/load/')) {
      return Promise.resolve(
        jsonResponse(opts.load ?? { fields: {}, path: null })
      );
    }
    const post =
      opts.post?.() ??
...
describe('AuthoringForm (md_run)', () => {
  ...
  it('shows a backend 422 error inline on the field', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({
          errors: [
            { loc: ['seed'], msg: 'must be an integer', type: 'int_parsing' }
          ]
        }),
  ...
  it('auto-loads the run from the edit-link search param (deep link)', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText(/Seed/)).toHaveValue(42));
...
describe('AuthoringForm (acquisition)', () => {
  ...
  it('adds and removes tilt-series entries', async () => {
    ...
    await userEvent.click(
      screen.getByRole('button', { name: /Add tilt series/ })
    );
  ...
  it('post_processed_tomogram derived_from offers in-form tomogram ids', async () => {
    ...
    await userEvent.click(
      screen.getByRole('button', { name: /Add raw tomograms/i })
    );
    await userEvent.type(
      screen.getAllByLabelText(/Tomogram id/)[0],
      'tomo_raw'
    );
    await userEvent.click(
...
describe('AuthoringForm (sectioned) save to file share', () => {
  const inMount = {
    fields: { seed: 42 },
    path: '/groups/cryoet/cryoet/MdRuns/run01'
  };
